...
test("answer refuses a PERMISSION prompt — replying with text would silently deny it", async () => {
  // `{kind: "confirmAction", value: "yes"}` carries no `approved` field, so the
  // server reads it as *not* approved: the user types an answer and unknowingly
  // denies the tool call. Refusing is the only safe behaviour.
  const r = await run([SID, "yes please"], {
    srvRequests: [
      { id: "srv-1", kind: "confirmAction", sessionId: SID, title: "Run rm -rf build?" },
    ],
  });
  assert.notEqual(r.code, 0);
  assert.match(r.err, /approve/, "and it names the verb that does fit");
  assert.equal(r.responses.length, 0, "nothing is sent — no silent deny");
});

...
test("approve refuses an INPUT prompt instead of answering the wrong shape", async () => {
  // `{kind: "input", approved: true}` answers a question that asked for a string
  // with a boolean: the agent gets no value and the user believes they replied.
  // The mismatch is worse in the other direction (see answer.test.ts), so both
  // verbs refuse rather than guess.
  const r = await run([SID], {
    srvRequests: [{ id: "srv-1", kind: "input", sessionId: SID, title: "Which database?" }],
  });
  assert.notEqual(r.code, 0);

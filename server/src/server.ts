...
export function startWsServer(opts: ServerOpts) {
  ...
  function handleHello(state: ClientState, env: Envelope) {
  ...
  async function handleCmd(state: ClientState, env: Envelope) {
    ...
    try {
      ...
      sendErr(state, env.id, (e as Error).message);
    }
  }
  ...
  function sendSnapshots(state: ClientState) {
  ...
  function broadcastSessionsSnapshot() {
    ...
    for (const c of clients.values()) if (c.authed) send(c, env);
  }
  ...
  function sendErr(state: ClientState, id: string, message: string) {

        ...
        try {
            switch (method) {
                case "select": {
                    ...
                    if (resp.kind === "askUserQuestion" && !resp.cancelled && resp.answers?.[0]) {
                ...
                case "confirm": {
                    ...
                    if (resp.kind === "confirmAction" && !resp.cancelled) {
    ...
    handleLine(line) {
        ...
        switch (evt.type) {
            ...
            case "tool_execution_update": {
                ...
                if (!callId || evt.partialResult === undefined)
                    return;
                this.emitEvent({
                    ts: Date.now(),
 ...
 * inside JSON strings.
 */
function bindLfLines(stream, onLine) {
    let buf = "";
...
function summarizeText(toolName, text) {

 ...
 */
import { mkdirSync, readdirSync, symlinkSync, writeFileSync, readFileSync, rmSync, existsSync, } from "node:fs";
 ...
 */
export function buildFilteredAgentDir(excludePackages) {
    ...
    if (Array.isArray(settings.packages)) {
        settings.packages = settings.packages
            ...
            return !excludePackages.some((ex) => p === ex ||
                p === `npm:${ex}` ||
                p.endsWith(`/${ex}`));
        })
            ...
            .map((p) => p.startsWith("npm:") || isAbsolute(p) ? p : resolve(src, p));
    }
    writeFileSync(join(dest, "settings.json"), JSON.stringify(settings, null, 2));
    return dest;
}
//# sourceMappingURL=pi-agent-dir.js.map
...
async function main() {
    ...
    if (extensionPaths.length === 0) {
        ...
        console.log(`[pino] loading ${extensionPaths.length} connector(s):`, extensionPaths.map((p) => p.split("/").pop()).join(", "));
    }
    manager.setBridge({
        url: bridge.url,
        token: bridge.token,
    ...
    try {
        ...
        console.error(`[pino] control socket unavailable: ${e.message}`);
    }

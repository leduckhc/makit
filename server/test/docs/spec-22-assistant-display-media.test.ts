...
// SPEC-22 design document. The PR under test adds only a markdown spec file
...
const SPEC_PATH = join(SPECS_DIR, "2026-07-20-SPEC-22-assistant-display-media.md");
const SPEC_FILENAME = "2026-07-20-SPEC-22-assistant-display-media.md";
...
test("SPEC-22 file exists at the expected docs/specs path", () => {
  assert.ok(existsSync(SPEC_PATH), `expected spec file at ${SPEC_PATH}`);
});

test("SPEC-22 filename follows the YYYY-MM-DD-SPEC-##-title.md convention", () => {
  const pattern = /^\d{4}-\d{2}-\d{2}-SPEC-\d+-[a-z0-9-]+\.md$/;
  assert.match(SPEC_FILENAME, pattern);
});

// -------- header / front matter --------------------------------------------

test("SPEC-22 has the expected title header", () => {
  const content = readSpec();
  const firstLine = content.split("\n", 1)[0];
  assert.equal(firstLine, "# SPEC-22 — Assistant display media (images, video, gifs)");
});

test("SPEC-22 front-matter block declares Status, Depends on, and Blocks", () => {
  const content = readSpec();
...
test("SPEC-22 declares its dependencies on SPEC-15 and SPEC-16", () => {
  const content = readSpec();
  assert.match(content, /SPEC-15 \(adapter consolidation\)/);
  assert.match(content, /SPEC-16\s*\n?\(app chat\)/);
});

// -------- required top-level sections ---------------------------------------

test("SPEC-22 defines all required top-level sections", () => {
  const content = readSpec();
...
test("SPEC-22 Scope section has In and Out subsections", () => {
  const content = readSpec();
...
test("SPEC-22 enumerates all four EventKind registration points with concrete file paths", () => {
  const content = readSpec();
  const expectedPaths = [
...
test("SPEC-22 flags that the codec has no Zod schema validation for media payload fields", () => {
  const content = readSpec();
...
test("SPEC-22 names the WS client fingerprint file as the pinning source to extract", () => {
  const content = readSpec();
...
test("referenced dependency specs (SPEC-15, SPEC-16) exist in docs/specs", () => {
  const files = readdirSync(SPECS_DIR);
  const hasSpec15 = files.some((f) => /SPEC-15-/.test(f));
  const hasSpec16 = files.some((f) => /SPEC-16-/.test(f));
  assert.ok(hasSpec15, "expected a SPEC-15 file to exist in docs/specs");
  assert.ok(hasSpec16, "expected a SPEC-16 file to exist in docs/specs");
});

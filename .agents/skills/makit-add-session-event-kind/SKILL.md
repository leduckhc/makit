...
- High-frequency events must join `NO_FANOUT_KINDS` in session.ts or every update re-broadcasts the whole sessions snapshot (SPEC-server-hotpath-and-state P2).
- `SpawnOpts.extensions` is NEVER forwarded to pi — pi-acp spawns `pi --mode rpc` itself, so there is no `-e` channel. Only env vars propagate. A pi extension therefore needs a symlink into `~/.pi/agent/extensions/`, the way `makit serve` installs the mirror extension (SPEC-computer-use, SPEC-context-usage). That link is persistent and loads whatever it points at, so point it at a stable clone, never at a throwaway worktree — see `makit-computer-use`. Remove a link you added only for a probe.

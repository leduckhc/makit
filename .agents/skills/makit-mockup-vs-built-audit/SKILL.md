...
2. Write a scratch golden harness under `app/test/sim/`. Screenshot with `expectLater(find.byType(MaterialApp), matchesGoldenFile('images/<name>.png'))` and run `flutter test --no-pub --update-goldens test/sim/<file>.dart`. Set `tester.view.physicalSize = Size(logicalW * dpr, logicalH * dpr)` with dpr 2 for macOS panes and 3 for iPhone (393pt), and `addTearDown(tester.view.reset)`.
3. In `setUpAll`, load real fonts or every glyph renders as an Ahem box: `/System/Library/Fonts/SFNS.ttf` registered under 'SF Pro Text' (the app's `kSansFontFamily`), 'Roboto' and '.SF NS', plus `~/.pub-cache/hosted/pub.dev/phosphoricons_flutter-1.0.0/lib/fonts/Phosphor-*.ttf` under `packages/phosphoricons_flutter/Phosphor<Weight>` and `assets/fonts/codicon.ttf`.

...
- **Check what the spec told the screen to reuse.** SPEC-doc-preview line 115 maps the Docs screen to `app/lib/ui/ports/ports_screen.dart`, so its chrome must be diffed against the *shipped Ports screen*, not the mockup: `←` back arrow, `title · subtitle`, ✓-marked selected chip with plain counts, `UPPERCASE-DIRNAME <count>` group header, branch-icon sub-header, lowercase status pills. Deviations that match Ports are consistency wins; only deviations from BOTH the mockup and the reused screen are defects.

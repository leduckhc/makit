...
## Procedure
1. 1. Add socket interception to `makit-server` startup (before adapter.start) to tap the raw ACP JSON-RPC stream
2. 2. Prompt the user to interact with pi-acp: run multiple tool calls (edit a file, read, grep, bash, ask a question), trigger thinking, let sub-agents run
3. 3. Capture all `session/update` frames (not tool.call.*, just raw ACP frames) into a `.jsonl` file indexed by seq
4. 4. Create a `SessionFixture` TypeScript class that implements the ACP transport interface but replays captured frames
5. 5. Write unit tests that feed the fixture through `AcpEventMapper` and assert the resulting chat_items match expected one-liners, diff bodies, and lifecycle
6. 6. Dart-side test: mount `SrvRequestHandler` + `chat_transcript`, feed app events from fixture replay, take screenshots or assert widget tree

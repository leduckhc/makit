...
2. Before every macOS run, clear the cache from one verified root: `cd "$(git rev-parse --show-toplevel)/app" && rm -rf .dart_tool/flutter_build build/macos/Build/Products/Debug`. Resolve the root first — a relative `rm -rf` run from the wrong cwd either misses or hits the wrong tree. Even then, verify the binary is fresh by reading the harness's own top-bar stamp (`tool one-liner · <brightness> · <width>pt`) — a stale macOS bundle silently ran with different State defaults than the source and made the first QA pass grade the wrong theme.
...
5. Drive the harness by element token, which needs pid + window_id as well: `cua-driver call click '{"pid":P,"window_id":W,"element_token":"sXXXX:2"}'`. An element-token click delivers in the background and leaves the user's foreground alone; add `"delivery_mode":"foreground"` only for a click that does not land. Tokens are re-issued on every snapshot, so fetch them immediately before each click. This is how you reach light/dark, 430/560/full, ruler, and how you expand a row.
...
7. Capture iOS against the simulator's own ID, never `booted`: `sim=$(xcrun simctl list devices booted -j | jq -r '[.devices[][].udid]|first')`, then `xcrun simctl ui "$sim" appearance dark` and `xcrun simctl io "$sim" screenshot out.png`. With two simulators booted, `booted` is ambiguous and can shoot the wrong one. Set the appearance first: a freshly erased sim boots light, which would make every colour claim wrong.
...
4. `flutter analyze --no-pub` clean, and the full `flutter test --no-pub` run has no failure outside the `loading <file> [E]` stage. Each touched test file is also green on its own. An isolated pass alone is not sign-off: name every accepted `loading` flake.

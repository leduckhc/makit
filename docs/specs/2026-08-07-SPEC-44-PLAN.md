# SPEC-44 — Implementation plan (P4)

Spec: [`2026-08-07-SPEC-44-ports-forward.md`](./2026-08-07-SPEC-44-ports-forward.md) ·
...
Green: `PortsService` gains `{ isWatched, onPortDown }` (injected, like SPEC-41's other
...
`send`, matching SPEC-41 deviation #3 — no `request` ack timer on a UI toggle); the toggle in
...
SPEC-41-PLAN / SPEC-34-PLAN use). Empty at the start.

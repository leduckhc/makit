# SPEC-48 — Activity: one durable, copyable record of everything the app tells you

**Status:** Draft · **Priority:** P2 · **Branch:** `feat/status`
**Depends on:** SPEC-07/08 (`app/lib/notifications/` — actionable + background-wake OS
notifications), the diagnostics slice (`app/lib/diagnostics/log.dart` — `MakitLog`, the ring
buffer + stream + sinks shape this spec copies), SPEC-41/43/44 (`ports_vocabulary.dart`,
...
| `NotificationService` (SPEC-07/08) | ✅ | user, **absent** | ❌ fire-and-forget | ❌ |
...
only a request id), i.e. changing the path SPEC-08 made idempotent, to record a fact the
...
- No persistence (D9), no server round-trip: activity is a *client* record of what this
  client did and saw.
- No new OS notification triggers. SPEC-07/08 decide what buzzes your phone; this spec only

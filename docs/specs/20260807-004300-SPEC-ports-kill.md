# SPEC-ports-kill — Ports P3: kill a listener you can see, safely

**Status:** Draft · **Priority:** P2 · **Branch:** `feat/open-ports-kill`
**Depends on:** SPEC-open-ports (open ports — scan, attribution, `PortDTO`/`PortsSnapshotDTO`,
`ports.watch`, the popover + the two mobile sheets), SPEC-decomposition-and-dedup (`ws/commands/*` split,
`CommandRouter`, `CommandDeps`), SPEC-background-wake-notifications (paired-device bearer + `DeviceRegistry`).
**Soft-depends on:** SPEC-ports-global-view (orphan + collision detection via port history) — required **only**
...
SPEC-open-ports P1 shipped the scanner and said, in as many words:
...
sends a signal to the wrong process. SPEC-open-ports's **D6** is load-bearing:
...
## Phasing (this spec is internally phased so P3a ships without SPEC-ports-global-view)

| Phase | Content | Depends on |
| --- | --- | --- |
| **P3a** | Single-port `ports.kill` — D1 re-verify, D2 escalation, D3 whitelist, D4 auth, D7 audit, D8 confirm; desktop popover `Kill`, mobile sheet-2 `Kill`. **Independently shippable.** | SPEC-open-ports only |
| **P3b** | `ports.killOrphans` (D5) + the orphans section's `Kill all orphans (n)` button (§6). | **SPEC-ports-global-view** (orphan set) |
| — | Suggested free port (D6). | moved to **SPEC-ports-global-view** |

**P3a is useful without SPEC-ports-global-view:** a wedged dev server whose worktree still exists — the §6
...
returns `not_owned` until SPEC-ports-global-view supplies the orphan set; that is the crisp, stated limitation.

## What P3 does not do

- **No auto-kill.** SPEC-open-ports §10 rejects *"auto-killing idle servers … too clever, too destructive
...
- **No inline kill button in the mobile list** (§10 fat-finger trap) — sheet 2 only, last, behind
  the divider.
- **No `Stop container`** (the mockup's docker row action). Docker attribution is SPEC-ports-global-view; without
  it there is no container to name, so it is deferred with docker.
- **No suggested-free-port** (D6 → SPEC-ports-global-view) and **no kill-all-orphans before SPEC-ports-global-view** (D5).
...
export type CmdKind =
   ...
   */
  | "ports.kill"
  /** SPEC-ports-kill P3b (needs SPEC-ports-global-view's orphan set): kill every current orphan (D5). */
  | "ports.killOrphans";
```
...
| **R4** | the matched pid has **no `worktreePath`** (and is not a SPEC-ports-global-view orphan) | `not_owned` | drop the ownership check → test a system listener (e.g. `sshd`, no `worktreePath`) → **no signal** |
...
Constants (named, one place, with comments — SPEC-open-ports's convention):
...
- **`PortsService.killOrphans()` (P3b)** maps `killPort` over the orphan set SPEC-ports-global-view supplies;
...
| `ports/kill_acceptance.test.ts` | **real-machine proof:** temp git worktree + a real `node` listener with its `cwd` in it → real scan → `killPort` the real tuple → the port is gone and a **second** `killPort` returns `not_found`. `test.skip` with a printed reason where `lsof` is unavailable (Linux CI parity, per SPEC-open-ports T9) | — |

P3b (`ports.killOrphans`) tests are written but gated on SPEC-ports-global-view's orphan source; until it lands the
...
3. `ports/kill_acceptance.test.ts` — the scripted real-listener-in-a-real-worktree kill, on macOS
   **and** Linux (the two emit different `lsof -F` records; the SPEC-open-ports T9 lesson).

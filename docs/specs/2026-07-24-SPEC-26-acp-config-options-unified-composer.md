# SPEC-26 — ACP session config options + unified composer config model
...
**Depends on:** SPEC-15 (server adapter consolidation), the ACP transition work (`docs/ACP_TRANSITION.md`). *Foundation for SPEC-27 (new-session config) and SPEC-28 (workspace tabs), which build on this config model.*
...
— not after a turn. SPEC-27 **caches** each harness's `configOptions` (populated
...
- **Timing-agnostic plumbing.** This spec covers the adapter + composer config
  model; the ACP config catalog is **cached** by SPEC-27 (one-time throwaway
...
needs `cwd`) — but only **once**, for a cached catalog (SPEC-27), via a throwaway
...
- `server/src/adapters/pi.ts` — the native pi adapter; **removed** by SPEC-27
...
- pi is **not** a special case among ACP agents: it runs through `AcpAdapter`
  ...
  file-access is accepted — no extra fs sandboxing planned, see SPEC-27.)
...
3. **pi over ACP + codex projection.** (a) With a stub ACP adapter emitting
   ...
   path is **SPEC-27's** step; until then the legacy-meta fallback covers
   native pi.)
...
- **Migration window.** The legacy `model/thinking/modes` `SessionMeta` fields
  ...
  path is deleted outright (SPEC-27).
...
- **`pi-acp` file access (accepted).** Reintroducing `pi-acp` reopens the ACP
  ...
  additional fs sandboxing is planned** (SPEC-27).

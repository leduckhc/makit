...
| T5b mobile on/off | ⬜ deferred with T8 (nothing to switch on yet) |
| T8 scrubber · T9 outline · T10 breadcrumb · T11 palette | ⬜ not started (visible but disabled in Settings) |
| T12 docs sweep | ⬜ not started |
...
4. **The landing flash has no `Timer`.** A pending timer trips `flutter_test`'s

...
> that ship — the desktop rail and the mobile messages sheet — deleting the
> palette, breadcrumb and outline styles, their options/preferences, and the
> settings style picker. T5, T8, T9, T10 and every reference below to those styles
> (or to `message_navigator_outline_test.dart`) describe work that existed and was
> removed; see the banner on the spec. T1–T4, T6, T7 and the sheet stand.

# SPEC-worktree-scoped-panes — Worktree-scoped pane layouts

**Status:** Superseded by [SPEC-desktop-workspace-tabs](./20260724-002800-SPEC-desktop-workspace-tabs.md) · **Priority:** P2 · **Surface:** desktop chat (`app/lib/desktop/chat/`)
**Depends on:** SPEC-session-in-pane-spawning (session-in-pane spawning), SPEC-desktop-chat-app (desktop chat app), SPEC-decomposition-and-dedup (pane decomposition — the files this reshapes)

> **Superseded.** The worktree-owns-the-layout model below is replaced by
> SPEC-desktop-workspace-tabs, which makes one worktree-agnostic **workspace** with recursive
> **splits** and **tabs** (a split hosts a tab strip; tabs can host sessions
> from any worktree). See SPEC-desktop-workspace-tabs → "Relationship to SPEC-worktree-scoped-panes".

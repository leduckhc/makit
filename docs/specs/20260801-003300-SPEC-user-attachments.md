# SPEC-user-attachments — User attachments (picker + clipboard paste)

**Status:** implemented (phase 1: images, file hand-off, picker + paste) · **Date:** 2026-08-01 · **Depends on:** SPEC-assistant-display-media (assistant
display media — store, route, pinned client, renderer), SPEC-acp-config-options-unified-composer (unified
composer), SPEC-session-lifecycle-resume-list-delete (adapter capability negotiation) · **Blocks:** —

> SPEC-assistant-display-media explicitly deferred this: *"Not built: … user→agent uploads (pi
...
return const IconButton(
  ...
  onPressed: null,          // ← disabled since SPEC-composer-adaptive-input
);
```
...
SPEC-assistant-display-media built the hard, security-sensitive half of this feature for the
...
| 32 MB child-RPC frame cap (base64 payloads fit) | `server/src/adapters/child_transport.ts` | already raised by SPEC-assistant-display-media |

The remaining work is a **transport verb, a wire field, two prompt builders, and
composer UI**. The blast radius is smaller than SPEC-assistant-display-media's.
...
- Capability is per **agent**, but image support is really per **model** (an ACP
  ...
  cannot see pictures). SPEC-acp-config-options-unified-composer/31 already model per-active-model config, so the
...
Descriptors only — the same reason SPEC-assistant-display-media gives (replayed log). The app's
...
- Video/audio attachments (SPEC-assistant-display-media phase 4 is still unbuilt on the output side).
- Media GC/refcounting — inherited SPEC-assistant-display-media debt, now with a second producer;
  worth its own spec.
...
- **O4** (new, from the §3.4b deferral) — where does "this *model* can see
  images" come from? SPEC-model-picker-menu-per-model-config gives us per-active-model config, but no model

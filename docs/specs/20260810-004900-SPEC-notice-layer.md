# SPEC-notice-layer — The notice you can copy and re-read (rev 2)

**Status:** Implemented (rev 2 — after two independent reviews) · **Priority:** P1 · **Branch:** `feat/status`
**Depends on:** SPEC-status-and-activity (`app/lib/status/` — this spec changes only how its notice is *presented*),
SPEC-tab-groups (`app/lib/shortcuts/`) for the keyboard path.
...
building a second notification system. SPEC-status-and-activity already moved the notice off the bottom-centre
...
SPEC-status-and-activity D9 stands; dismissal removes the view, never the record.
...
- **It does not add transcript rows** for session failures (the mockup's structural idea). Its
  precedent is real (`ask_card.dart`, SPEC-ask-user-inline-in-chat) and so is its cost: a local ephemeral row kind in

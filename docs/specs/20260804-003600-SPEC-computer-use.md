# SPEC-computer-use — Computer use for pi and codex sessions

**Status:** Shipped (server-side enablement, opt-in) · **Priority:** P3 · **Branch:** `feat/computer-use`
**Depends on:** SPEC-assistant-display-media (assistant display media), SPEC-new-session-config-at-spawn (capability probe / spawn args)
...
The SPEC-new-session-config-at-spawn throwaway capability probes deliberately do **not** get these args — they
...
store (SPEC-assistant-display-media).

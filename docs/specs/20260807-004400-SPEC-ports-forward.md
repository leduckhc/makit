# SPEC-ports-forward — Ports P4: forward a loopback port to the phone, and watch a port

**Status:** Draft · **Priority:** P3 · **Branch:** `feat/open-ports-fixup`
**Depends on:** SPEC-open-ports (ports scan/attribution/`PortDTO`/`ports.watch`), SPEC-assistant-display-media + SPEC-user-attachments
(`server/src/media/route.ts` — authenticated HTTP over the WSS listener, the pinned
`HttpClient`, `trustLoopback`), SPEC-background-wake-notifications/08 (actionable push, `server/src/push/`), SPEC-repo-centric-home
...
| D4 | **A port may be forwarded only if it passes every rule**, each testable: (1) `reach === "loopback"`; (2) `worktreePath` is set; (3) `openUrl` is present (it answered HTTP under SPEC-open-ports's probe); (4) it is not the server's own `port`; (5) its port is not in `NO_HTTP_PROBE_PORTS` (22, 5432, 3306, 6379, 27017, 11211). A `ports.forward` that fails any rule is refused with a one-line reason, minting no grant. | (1) an `exposed`/`tailnet` port is already reachable — forwarding is pointless. (2)+(3) forwarding proxies HTTP, so forwarding something unowned or that never spoke HTTP is guaranteed-broken; owned + probed is the same trust boundary SPEC-open-ports D3 already drew. (4) self-proxy is a loop. (5) the database deny-list is exactly the "assumed loopback-only, no auth of its own" services the threat model (D6) is about — refused outright, not merely unprobed. |
...
**On SPEC-ports-global-view/43:** neither exists in the repo today, so **P4 declares no hard dependency on
them.** P4a builds only on SPEC-open-ports's published snapshot; P4b builds only on the SPEC-assistant-display-media/33
media-route transport. If SPEC-ports-global-view (a global Ports screen) or SPEC-ports-kill land later, the forward
...
```ts
// PortDTO gains one optional, backward-compatible field (SPEC-open-ports's tolerant
// decode drops what it doesn't know; an absent value means "not watched").
export interface PortDTO {
  // …all SPEC-open-ports fields unchanged…
...
session or host event. This keeps SPEC-open-ports's frozen `snapshots.json` golden valid (an absent
...
- **No global Ports screen, no menubar, no orphan/collision detection** — SPEC-ports-global-view territory.
...
| Layer | Test |
| --- | --- |
| `protocol/contract.test.ts` | SPEC-open-ports's `snapshots.json` golden still round-trips with `PortDTO.watched` absent; a snapshot **with** `watched:true` round-trips; `decodeSessionEvent` still rejects `ports.snapshot`; no new `HOST_ONLY_KINDS` entry |

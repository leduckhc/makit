# SPEC-desktop-control-app — macOS desktop control app

**Status:** done (Phase 1 via PR #11; Phase 4 via PR #16) · **Depends on:** SPEC-daemon-control-plane (control protocol) · **Blocks:** —

**→ Detailed architecture & work plan:** [`20260705-000300-SPEC-desktop-control-app-ARCHITECTURE-AND-PLAN.md`](./20260705-000300-SPEC-desktop-control-app-ARCHITECTURE-AND-PLAN.md)
...
list/revoke paired devices. It talks to the running makit daemon over the SPEC-daemon-control-plane
...
4. **Server lifecycle = drive the daemon.** The app runs `makit start` /
   ...
   One server implementation (SPEC-daemon-control-plane); the app does **not** embed a server.
...
### Out (v1)
- Daemon internals — SPEC-daemon-control-plane.
- Session-spawning-in-pane behavior — SPEC-session-in-pane-spawning.
...
- **Control plane** over the SPEC-daemon-control-plane unix socket at `~/.makit/control.sock`
...
- **Server lifecycle** via the CLI: `makit start` / `makit stop` / `makit restart`
  (SPEC-daemon-control-plane). The app spawns these with `Process.start` after CLI discovery.

# SPEC-queue-tray-and-promote — Queue tray: a second presentation, and promote

> **Renumbered from SPEC-performance-metrics-dashboard.** `main` landed its own SPEC-performance-metrics-dashboard (the performance
...
**Depends on:** SPEC-mid-turn-steering-and-queue (the queue), SPEC-pending-queue-edit-reorder (edit / reorder / placements)
...
SPEC-pending-queue-edit-reorder shipped variant **E** (ghost bubbles) twice — `pinned` and `inline` —
...
2. **`inline` is gone, not kept alongside.** The queue now has exactly ONE mount
   ...
   trailer is back to what SPEC-chat-scroll-anchoring/34 defined: ask card or working indicator, and
...
- **Drag to reorder.** The tray keeps ↑↓ for the same reason the bubbles do
  (SPEC-pending-queue-edit-reorder decision 6): both sit inside or beside a scrollable.
- **A per-message steer.** Steering is chosen by capability, not by the user
  (SPEC-mid-turn-steering-and-queue); promote is the only per-message *delivery* choice.

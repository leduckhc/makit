# SPEC-daemon-control-plane — makit daemon & local control plane

**Status:** done · **Depends on:** none (foundation) · **Blocks:** SPEC-cli-client-subcommands, SPEC-desktop-control-app

## Goal

Decouple the running makit server from any single foreground terminal. Introduce
a **user-controlled background service** (explicit start/stop) plus a **local
control plane** (unix-domain socket) so other processes — the CLI (SPEC-cli-client-subcommands) and
the desktop app (SPEC-desktop-control-app) — can drive a *running* makit: show/refresh the QR,
re-pair, list/revoke devices, list sessions, tail logs, and stop the server —
**without restarting it**.

## Why

Today `makit serve` is a foreground process. You cannot ask a running instance to
...
### In
- Background lifecycle: `makit start`, `makit stop`, `makit restart`, `makit status`,
  `makit logs [-f]`.
...
- **Control socket**: a unix-domain socket at `~/.makit/control.sock` (mode 0600)
...
- PID file `~/.makit/makit.pid`, log file `~/.makit/makit.log` (rolled or truncated
  on start — keep simple: truncate, document it).
- Opt-in OS integration: `makit service install` / `service uninstall` writes/removes
  a macOS **launchd LaunchAgent** plist (`~/Library/LaunchAgents/dev.makit.plist`)
  with `RunAtLoad`/`KeepAlive=false`. Default install does nothing automatically
  (consensus #2: user starts/stops on demand).
- Keep `makit serve` working unchanged (foreground) for headless servers / debugging.

### Out
- CLI client verbs like `makit qr`/`makit devices` — those are **SPEC-cli-client-subcommands** (they
  are *clients* of this control socket).
- Any GUI — **SPEC-desktop-control-app**.
...
- New module `server/src/daemon/` :
  ...
  - `control-client.ts` — a tiny reusable client (`connect + request(verb, args)
    → response`) used by SPEC-cli-client-subcommands. Export it.
...
### Control protocol (freeze this — SPEC-cli-client-subcommands/03 depend on it)
...
| `pair.mint` | `{ ttlMs? }` | `{ url, token, expiresAt, fingerprint }` — mint a fresh pair token + build the makit:// URL |
...
- [x] `makit start` returns immediately; server keeps running after the launching
      shell exits; `~/.makit/makit.pid` and `makit.log` exist.
- [x] `makit status` (calls `status` verb) prints pid/port/fingerprint/paired
      count when running; prints "not running" + exit code 3 when not.
- [x] `makit stop` triggers graceful shutdown; PID file removed; socket closed.
- [x] `makit restart` = stop (if running) + start; survives.
...
- [x] `makit serve` (foreground) still works and now **also** serves the control
      socket (so `makit status`/`qr` work against a foreground instance too).
- [x] `makit service install` writes a valid launchd plist; `uninstall` removes
...
- Does `server.stop` also stop sessions/panes? For v1: stop the server process;
  SPEC-session-in-pane-spawning owns pane teardown on its own session-end path.

# SPEC-actionable-notifications — Slice 1: Actionable notifications (resolve approvals from the lock screen)

**Status:** implemented · **Depends on:** none
**Fast-follow:** [SPEC-background-wake-notifications](./20260708-000700-SPEC-background-wake-notifications.md) (force-quit wake)
...
SPEC-background-wake-notifications. This plan marks the exact seam where the killed-app path plugs in.
...
isolate with no socket/Riverpod, i.e. the SPEC-background-wake-notifications problem. Building its
queue+replay now is YAGNI.

**SPEC-background-wake-notifications seam:** register `onDidReceiveBackgroundNotificationResponse` as a
...
replay; SPEC-background-wake-notifications drains this on next launch/reconnect through the same
`responseForAction` + `respondTo` path. Mark with `// SPEC-background-wake-notifications:`.
...
1. **Locked-screen foreground action** may require unlock before running; truly
   silent locked Approve needs the background isolate → SPEC-background-wake-notifications. Accept for
   Slice 1; document.
...
5. **SPEC-background-wake-notifications queue store** — `SharedPreferences` (non-secret action queue), not
   `flutter_secure_storage`. Finalize when SPEC-background-wake-notifications is scoped.

# SPEC-multiplexer-adapter-layer — Multiplexer adapter layer + herdr

**Status:** implemented · **Depends on:** none (foundation) · **Blocks:** SPEC-session-in-pane-spawning
...
- Deciding *when* to spawn a session in a pane, correlation with a makit session,
  and auto-close-on-end — that's **SPEC-session-in-pane-spawning** (this spec only provides mechanism).
- tmux/cmux implementations — interface must accommodate them; do not build.

## Interface (freeze — SPEC-session-in-pane-spawning depends on it)
...
- **Config file shape:** `~/.makit/config.json` → `{ "mux": { "name": "herdr", "anchor": "makit" } }`.
  ...
  the registry (returns `undefined` — used by SPEC-session-in-pane-spawning fallback).
...
- **`setLabel` optional:** not every future mux may support rename. `HerdrAdapter`
  implements it; SPEC-session-in-pane-spawning title updates must use `adapter.setLabel?.(...)` so

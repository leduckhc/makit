# SPEC-cli-as-client — The CLI is a client: sessions from the terminal, handoff as a command
...
**Depends on:** SPEC-daemon-control-plane (daemon + control socket), SPEC-cli-client-subcommands (CLI subcommands — the dir, the
`requireDaemon()` convention, exit 3), SPEC-new-session-config-at-spawn (drafts: a spawned session defers its worktree and
harness until the first message; config picks at spawn), SPEC-session-lifecycle-resume-list-delete (adapter-native lifecycle —
resume/list/**fork pending**), SPEC-user-attachments (attachments), SPEC-mid-turn-steering-and-queue/38/39 (queue + steering).
...
| Fork a conversation | `SessionCapabilities.fork`, `AgentAdapter.forkSession?()`, codex `thread/fork` | **`session.fork` cmd never landed** (SPEC-session-lifecycle-resume-list-delete marks it PENDING) |
...
1. **Split-brain transport.** `makit sessions` speaks the **control socket** (`sessions.list`,
   ...
   second set of DTO projections to drift — exactly the duplication SPEC-decomposition-and-dedup spent a spec deleting.
...
| **D12** | **No TUI, no `race`, no `fleet`.** `makit attach` stays the line-oriented readline client it is. | SPEC-cli-client-subcommands already put a long-running TUI out of scope, and ensemble/fleet orchestration is composition *on top of* this verb set — it should be specced once these verbs are real, not designed against imagined ones. |
...
| `makit fork <id> [--agent A]` | `session.fork`† (completes SPEC-session-lifecycle-resume-list-delete) | P2 |
...
- **P1 — the handoff workflow.** D3 (env identity), D4 (`new`), D5 (`handoff`), D9 (depth guard),
  ...
  ritual.**
- **P2 — the terminal as a full peer.** `session.fork` (SPEC-session-lifecycle-resume-list-delete's pending item), `approve`/`answer`
...
- [x] Daemon down → SPEC-cli-client-subcommands's message and exit `3`, no stack trace, for every verb.
...
2. **Where does the manifest live on disk?** `<worktree>/.makit/handoff/*.json` (git-excluded, like
   SPEC-user-attachments's materialised attachments, and visible to both agents) vs `~/.makit/handoff/`
   (server-scoped, survives worktree removal).
3. **`makit sessions` alias lifetime** — one release, or keep it permanently since it is in SPEC-cli-client-subcommands's

# SPEC-performance-metrics-dashboard — Performance dashboard (prove the efficiency claim, then optimise against it)

**Status:** Draft · **Priority:** P2 · **Branch:** `feat/performance-metrics`
**Depends on:** SPEC-github-gateway-and-budget (footer popover + top-level broadcast-event precedent), SPEC-decomposition-and-dedup (`ws/commands/*` registry), SPEC-session-lifecycle-resume-list-delete (live session registry), SPEC-desktop-settings-rework (in-window overlay surface)
...
One new top-level event kind, following SPEC-github-gateway-and-budget's `github.budget` precedent
...
silently dropping their rows is precisely how SPEC-github-gateway-and-budget's PR pills used to vanish under
...
Charts are hand-painted for the same reason SPEC-github-gateway-and-budget's sparkline was: three shapes
...
derivation (SPEC-tab-groups), auto-select and pruning. That is a large, risky change to
...
| 8 | Charts are `CustomPainter`, no chart package | Three shapes; SPEC-github-gateway-and-budget's sparkline set the precedent |
...
| 13 | A failed `ps` sets `procTableOk: false` and **omits** the agent/app rows; the UI says *measurement unavailable* | Zeros read as "idle", missing rows read as "exited". Neither is true, and the second is the SPEC-github-gateway-and-budget vanishing-pill defect wearing a new hat |
...
| 17 | **`turnActive` is the authoritative turn signal**, not the presence of an in-turn agent row; a turn in an unmeasurable session reads *"turn running — not attributable to a measured process"* | Decision 11 omits pidless sessions from `agents` while the collector still reports `turnActive` for them, so deriving turn state from the rows printed *"idle"* while the footer glyph animated as Working — one panel asserting two contradictory things. The rows may refine the copy; they may not decide whether a turn exists |
| 19 | The single-overlay invariant lives in **`DesktopWindowBody`**, the one widget that hosts both surfaces — not in the providers, and not at the call sites | `listenSelf` is unavailable on the `Ref` a legacy `StateProvider` receives, and rewriting both flags as notifiers would churn eight existing call sites for no behavioural gain. Declaring each flag beside its own widget also created an import cycle that the analyzer resolves to `dynamic`, silently unchecking every `ref.read` of them — so both flags live together in `window_overlays.dart` and the host applies the rule |
| 18 | Frame timings are collected **only while a surface is watching** (registered with the 1 Hz watch, removed on release) and the row reads `—` until a frame is timed; the panel does **not** rebuild per frame | A leaked `addTimingsCallback` runs for the process lifetime, and a per-frame notifier would rebuild this panel at 60 fps — the meter becoming the cost. The row therefore refreshes at sample cadence (~1 Hz), which is the same cadence as every other number beside it |

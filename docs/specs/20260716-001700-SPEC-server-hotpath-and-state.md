# SPEC-server-hotpath-and-state — Server hot-path performance & session state model
...
- **Fix:** `Promise.all` over projects; within a project `Promise.all` over
  ...
  if needed. Extracting a `RepoService.listRepos()` (see SPEC-decomposition-and-dedup D-manager) makes

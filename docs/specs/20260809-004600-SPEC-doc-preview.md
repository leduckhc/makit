# SPEC-doc-preview — Docs: preview the HTML and Markdown that live in the repo
...
answer is therefore not a file browser — it is the Ports pattern (SPEC-open-ports/42) applied to documents:
...
| D3 | **`key` is `<worktreePath>:<relPath>` — a snapshot key, never persisted.** The UI re-selects by `(worktreePath, relPath)`. | Same reasoning as `PortDTO.key` (SPEC-open-ports D6). Files are renamed and moved; a stored key rots silently. Calling it an `id` invites callers to store it. |
| D4 | **The title is extracted, never the filename**: `<title>` for HTML, the first `# H1` for markdown, basename only as a last resort. | `20260807-004400-SPEC-ports-forward.md` is unreadable on a 375 pt row. *"SPEC-ports-forward — Ports P4: forward a loopback port to the phone"* is the actual name of the thing, and it is already in the file. This single decision is most of the perceived quality. |
...
| **P3** | `webview_flutter` + the pinned loopback proxy **shared with SPEC-ports-forward** → HTML in-app; then **Canvas** (split pane on desktop, second page on mobile) with **live reload** off `worktree_watcher`. | `webview_flutter` |
...
| Grant record, TTL, idle reaping | SPEC-ports-forward D3 (to be implemented here first) |
...
export interface DocsSnapshotDTO {
  docs: DocDTO[];
  scanOk: boolean;             // "the walk ran", not "the list is complete" (SPEC-open-ports D7)
  scannedAt: number;
}

/** An active publication. Same shape discipline as SPEC-ports-forward's forward grant (D9). */

...
Spec: [`2026-08-02-SPEC-38-pending-queue-edit-reorder.md`](./2026-08-02-SPEC-38-pending-queue-edit-reorder.md)
...
3. **`Composer.queued`/`onCancelQueued` were replaced by a single `pendingQueue` widget
   slot**, and SPEC-35's `queued_chips.dart` was deleted. The composer no longer knows the
   queue's commands, which is exactly what lets the *same* widget mount in the transcript.
   (Deleting my own SPEC-35 code, not someone else's.)

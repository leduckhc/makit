# SPEC-cli-client-subcommands — CLI client subcommands

**Status:** done · **Depends on:** SPEC-daemon-control-plane (control protocol) · **Blocks:** SPEC-desktop-control-app

## Goal

Give power users and headless/UI-less servers a complete CLI that drives a
**running** makit daemon via the control socket from SPEC-daemon-control-plane. Chiefly: reprint /
refresh the QR and re-pair **without restarting** the server, plus inspect and
manage devices and sessions.

## Scope

### In (new subcommands, all thin clients of the SPEC-daemon-control-plane control socket)
...
- `makit stop` — already defined in SPEC-daemon-control-plane as lifecycle; ensure it reads nicely.

### Out
- Daemon lifecycle/`start`/`service` — owned by SPEC-daemon-control-plane.
...
- New dir `server/src/cli/` already exists (`mirror.ts`). Add `qr.ts`,
  ...
  using SPEC-daemon-control-plane's exported `control-client`.

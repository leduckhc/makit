# SPEC-session-lifecycle-resume-list-delete — Adapter-native session lifecycle (resume · list · delete · fork)

**Status:** Proposed · **Priority:** P1 · **Surface:** server (`server/src/`), wire protocol (`protocol.ts`), Flutter store (`app/lib/store/`)
**Depends on:** SPEC-new-session-config-at-spawn (pi-over-ACP via `pi-acp`; codex over `app-server`; both projected into one config model), SPEC-server-hotpath-and-state (durable event log + rehydration), SPEC-decomposition-and-dedup (adapter/manager decomposition).
...
3. **`reattachSession()` is dead on arrival.** It requires
   ...
   removed in SPEC-new-session-config-at-spawn. Result: after a server restart **no** session can
...
> drops pending mid-turn input ("stop means stop", SPEC-mid-turn-steering-and-queue), and serializes
...
4. **Persist the native session/thread id, not a file path.** The session's
   ...
   **Legacy break:** rows persisted before this change that have only
   ...
   SPEC-new-session-config-at-spawn.
...
7. **Fork is exposed only where advertised.** A new `session.fork` cmd maps to
   ...
   dovetails with SPEC-desktop-workspace-tabs's split-session UI (fork the *conversation*, not just
...
- **Capability drift across versions.** Never assume; always read the
  ...
  supported `app-server` (the availability gate in SPEC-new-session-config-at-spawn already fingerprints
...
- SPEC-desktop-workspace-tabs split-session UI wiring for `session.fork` (server capability only
  here).

# SPEC-open-ports — Ports: what's listening, and whose branch owns it

**Status:** Implemented (P1, rev 2) · **Priority:** P2 · **Branch:** `feat/open-ports`
**Depends on:** SPEC-repo-centric-home (repo-centric home), SPEC-decomposition-and-dedup (`ws/commands/*` split, `repo_service.ts`),
SPEC-github-gateway-and-budget (watch-gated cadence precedent), SPEC-performance-metrics-dashboard (host-wide broadcast event, `metrics.watch`,
...
(`Session.agentPid`, SPEC-performance-metrics-dashboard) — so it is the only tool on this machine that can.
...
| session root pids | `Session.agentPid` — already how SPEC-performance-metrics-dashboard finds agent trees in `server.ts` |
...
and it runs on SPEC-performance-metrics-dashboard's 1 Hz hot path. Widening that tick's `ps` for a 4 s feature would tax
...
third and is the same call SPEC-performance-metrics-dashboard already affords at 1 Hz. Every exec carries a timeout.
...
SPEC-context-usage gave `StubAdapter` a usage ramp. `fake_server.dart` mirrors it for widget work.
...
| `tool/e2e.sh --mode=stub` | **not run** — needs an iOS simulator; the integration case is written and registered. Pre-existing harness breakage on this branch (same as SPEC-performance-metrics-dashboard recorded). |

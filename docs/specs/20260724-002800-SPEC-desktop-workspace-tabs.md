# SPEC-desktop-workspace-tabs — Desktop/iPad workspace: recursive splits + tabs

**Status:** Proposed · **Priority:** P2 · **Surface:** desktop chat (`app/lib/desktop/chat/`)
**Depends on:** SPEC-desktop-chat-app (desktop chat app), SPEC-decomposition-and-dedup (pane decomposition — the files this reshapes), SPEC-new-session-config-at-spawn (new-session dialog — the entry point that opens sessions as Tabs)
**Supersedes:** **SPEC-worktree-scoped-panes (worktree-scoped pane layouts)** — see "Relationship to SPEC-worktree-scoped-panes" below.
...
1. **Server execution slot** — `PaneInfo { mux, paneId }` (SPEC-session-in-pane-spawning, now
   **retired**): the terminal-multiplexer slot where a session ran (e.g. tmux
   `%1`). Backend-only, not visual. *With SPEC-session-in-pane-spawning/pi-over-ACP this concept is
...
Separately, the desktop layout model is limiting. SPEC-worktree-scoped-panes tied one split tree to
...
| **Tab** | one session view inside a Split. Carries `sessionId`, or (when empty) an optional worktree used only to pre-fill the SPEC-new-session-config-at-spawn dialog. | desktop + iPad |
...
> **No more "mux pane."** SPEC-new-session-config-at-spawn retires pi's server-side terminal-multiplexer
...
4. **A Tab is `{ id, sessionId? }`.**
   ...
   > in [SPEC-new-session-config-at-spawn](./20260724-002700-SPEC-new-session-config-at-spawn.md); it
...
7. **Empty workspace → placeholder.** On fresh launch (no saved layout) the
   ...
   session** button that opens the SPEC-new-session-config-at-spawn dialog.
...
## Relationship to SPEC-worktree-scoped-panes

SPEC-worktree-scoped-panes ("the worktree owns the pane tree") is **superseded and reverted** by
this spec:

- SPEC-worktree-scoped-panes's per-worktree keyed workspace (`Map<path, PaneTreeState>`,
  swap-on-select) is **removed**. There is one worktree-agnostic tree instead.
- SPEC-worktree-scoped-panes removed `PaneLeaf.worktree`; this spec restores a per-leaf worktree —
  now on the **Tab** (`Tab.worktree`), as the starter hint (decision 4). This is
  closer to the pre-SPEC-worktree-scoped-panes model, plus tabs.
- SPEC-worktree-scoped-panes's decision 4 ("selecting a session swaps to its worktree's tree") is
  replaced by decision 6 ("reveal the tab in the one shared layout").

If SPEC-worktree-scoped-panes was already implemented, this spec reshapes those files; if not, this
spec is implemented directly and SPEC-worktree-scoped-panes is marked superseded in the specs
README.
...
final class Tab {
  ...
  // SPEC-new-session-config-at-spawn New session dialog's Worktree field. It is NOT an inline picker
...
- A `Split` renders a **tab bar** (its `tabs`, active highlighted, close/reorder
  affordances, `+` opens the SPEC-new-session-config-at-spawn New session dialog) above the active Tab's
  body.
- The Tab body reuses `DesktopChatPane`: `sessionId != null` → chat; else the
  **empty placeholder** ("Select a session, or start a new one" + a **New
  session** button that opens the SPEC-new-session-config-at-spawn dialog, pre-filled with `tab.worktree`
...
- **Large test churn.** The pane tests encode single-session-per-leaf and (if
  present) SPEC-worktree-scoped-panes's per-worktree swap. They are rewritten, not tweaked —
...
- **Dead sessions after restart.** A persisted Tab may reference a session id
  ...
  state instead of crashing (mirror SPEC-worktree-scoped-panes's guard).

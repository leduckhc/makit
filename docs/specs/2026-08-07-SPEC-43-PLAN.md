# SPEC-43 — Implementation plan (P3a, single-port kill)

Spec: [`2026-08-07-SPEC-43-ports-kill.md`](./2026-08-07-SPEC-43-ports-kill.md) ·
...
free port (D6) do **not** ship here — P3b hard-depends on SPEC-42's orphan set (D5), D6 moved to
SPEC-42. P3a is independently shippable.
...
`PortKillOrphansResult` (P3b type declared now so the union is stable, unused until SPEC-42);
`CmdKind` += `"ports.kill"` and `"ports.killOrphans"` **with the P3b/SPEC-42 comment** so the next
...
`killPort(target): Promise<PortKillResult>` — **required**, per SPEC-41 deviation #2's lesson (an
...
Red-by-construction (mirrors SPEC-41 `ports/acceptance.test.ts`): create a temp git repo + worktree,
...
the listener in `finally` (cleanup must never throw and mask the original error — the SPEC-41 lesson).
...
| Kill-all-orphans ships before SPEC-42 can define "orphan". | D5: `ports.killOrphans` is declared but inert (empty orphan set) until SPEC-42; the button does not render; P3a stands alone. |

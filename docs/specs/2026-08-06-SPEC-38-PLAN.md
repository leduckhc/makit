...
**1733 app tests, 896 server tests, both analyzers clean.** They are recorded here because the spec
...
Phase 8  review loop (codex ×2, ocr) → fix verified findings        ✅ done
```
...
Verify: `flutter test test/ui/widgets/pr_signals_test.dart` — **56 tests**, pinning every row of spec
...
Verify: `flutter test test/desktop/chat/pr_bar_test.dart` — **25 tests**.
...
Verify: `flutter test test/ui/session/session_pr_test.dart` (**19**), `test/home_screen_test.dart`,
...
Final: **1751 app tests, 921 server tests**, `flutter analyze` and `pnpm typecheck` clean.
...
behaviour change, and `flutter analyze` + `flutter test` + `pnpm typecheck` + `pnpm test` green.

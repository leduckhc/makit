# SPEC-session-in-pane-spawning — Session-in-pane spawning + lifecycle

**Status:** **Retired** (was: done) — superseded by [SPEC-new-session-config-at-spawn](./20260724-002700-SPEC-new-session-config-at-spawn.md) · **Depends on:** SPEC-multiplexer-adapter-layer (mux adapter) · **Touches:**
`server/src/manager.ts`, `server/src/server.ts`, `server/extensions/makit-mirror.ts`,
app session DTO/UI

> **Retired.** SPEC-new-session-config-at-spawn drops the native pi adapter and runs **all harnesses
...
> is reversed by the pi-over-ACP decision (see SPEC-new-session-config-at-spawn Risks: this also reopens
...
> `PaneInfo`/mux-pane concept and this lifecycle should be removed with SPEC-new-session-config-at-spawn.

## Goal

When a **new session is created** (from the phone app or `makit`), makit should
launch `pi` in a **new background (unfocused) pane** of the active multiplexer
(SPEC-multiplexer-adapter-layer), which the user can attach to in the terminal. That `pi` mirrors to the
...
1. makit asks the mux adapter to open a background pane and run a `pi` launch
   command in the session's project cwd, passing a **correlation token** via env.
2. That `pi` auto-loads `makit-mirror` (already symlinked). The extension reads the
   correlation token and includes it in its `host.open` frame.
3. makit matches the incoming `host.open` (by token) to the pending spawn request,
   binds the resulting World-D session, and records the `PaneHandle` on it.
4. On session end/quit/`host.close`, makit calls `adapter.closePane(handle)`.

This means World-D `host.open` must carry a correlation id, and makit must be able
...
- **Correlation:** generate a `spawnToken` per pane-spawn; launch `pi` with
  `MAKIT_SPAWN_TOKEN=<token>` in the pane. Extend `makit-mirror`'s `host.open` to
  include `spawnToken` (read from `process.env.MAKIT_SPAWN_TOKEN`). Server matches
...
- **Spawn path:** `manager` gains a `spawnPiSessionInPane(projectId, opts)` (or a
  ...
  pane runs something like: `cd <cwd> && MAKIT_SPAWN_TOKEN=<t> pi` (respect
  `MAKIT_PI_BIN`). Background/unfocused.
...
- **Labeling:** set pane label to `makit: <session title>`; update it when the
...
- **Config/toggle:** honor a setting to enable/disable pane-spawning (default on
  when a mux is available); env `MAKIT_MUX=off` disables.

### Herdr host setup (prerequisite)

Pane spawning calls SPEC-multiplexer-adapter-layer's `HerdrAdapter.spawnPane`, which runs:

```bash
herdr pane split <anchor> --direction down --cwd <cwd> --no-focus
```

**Important (verified in SPEC-multiplexer-adapter-layer smoke test, herdr v0.7.1):** herdr's `pane split`
takes a **pane id** (e.g. `w1:p1`), not a workspace label. The SPEC-multiplexer-adapter-layer default
anchor `"makit"` is a *workspace label* — it does **not** resolve as a split
...
2. Create a dedicated workspace for makit sessions (once per host):

   ```bash
   herdr workspace create --cwd ~/your-project --label makit --no-focus
   ```
...
3. Set the split anchor to that workspace's **root pane id** (from the create
   ...
   ```bash
   export MAKIT_MUX_ANCHOR=w1:p1
   ```

   Or in `~/.makit/config.json`:
...
**SPEC-session-in-pane-spawning implementer note:** if `getMultiplexer()` is available but spawn fails
with `pane_not_found`, treat it like mux unavailable and fall back to headless
spawn (log a one-line hint about `MAKIT_MUX_ANCHOR`). Optional follow-up: auto-
bootstrap a `makit` workspace and cache its root pane id — out of scope for v1
...
```bash
cd server
MAKIT_MUX=herdr MAKIT_MUX_ANCHOR=w1:p1 tsx test/mux-herdr.smoke.ts
# → SPEC-multiplexer-adapter-layer herdr smoke: PASS
```
...
### Out
- The mux mechanism itself — SPEC-multiplexer-adapter-layer.
- Externally, user-launched `pi` (pure World D) is unchanged — it still
  self-registers with no pane handle (makit didn't spawn it, so it won't close it).

## Contracts touched
- `makit-mirror.ts`: `host.open` gains `spawnToken?: string` (from env). No behavior
...
- [x] Creating a session from the phone opens a **new unfocused herdr pane**
      ...
      mirrors that pi.
- [x] The pane is labeled `makit: <title>` and updates on rename.
...
- **Correlation transport:** env var (`MAKIT_SPAWN_TOKEN`) is simplest and robust.
  Confirm the mux `run` command can set env (herdr `pane run` runs a shell string,
  so `MAKIT_SPAWN_TOKEN=... pi` works). Alternative: `pi` CLI flag.
...
- **Which pi binary / args** the pane runs (respect `MAKIT_PI_BIN`, project cwd,
  and any resume flags). Ensure it does NOT set `MAKIT_BRIDGE_URL` (that env makes
  `makit-mirror` bail — see extension guard).
- **Herdr split anchor:** resolved — see **Herdr host setup** above. `MAKIT_MUX_ANCHOR`
  / `mux.anchor` must be a pane id (e.g. `w1:p1`), not the workspace label `"makit"`.

...
| [SPEC-37](./2026-08-04-SPEC-37-queue-tray-and-promote.md) | **Queue tray + promote** — mockup variant C, the compact work-list presentation of the pending queue, as a third value of the same placement preference (`pinned` · `inline` · `tray`). Adds `queue.promote`: interrupt the running turn so ONE queued message is delivered next, keeping the rest — composed from `reorderQueue` + `adapter.cancel()`, deliberately NOT from `cancel`, which clears the queue. A stale promote acks and does nothing, because aborting a turn on a late tap destroys work. Also renames the queue commands' message id to `queuedId`: as `id` it was silently overwriting the envelope's request id | SPEC-35, SPEC-36 |

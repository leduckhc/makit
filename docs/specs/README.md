...
| [SPEC-34](./2026-08-01-SPEC-34-message-navigator.md) ([plan](./2026-08-01-SPEC-34-PLAN.md)) | **Message navigator**: find your own messages in a long transcript. Five selectable styles (cosy ripple rail · prompt scrubber · prompt palette · sticky breadcrumb · outline mode) + off, each with per-style options in Settings › Agents & Chat. Markers are placed by **item index**, never scroll offset, because SPEC-21's reversed lazy list has no offset for un-built rows | SPEC-21, SPEC-24 |
...
| [SPEC-36](./2026-08-02-SPEC-36-pending-queue-edit-reorder.md) ([plan](./2026-08-02-SPEC-36-PLAN.md)) | **Pending queue: editable, reorderable, two placements** — a queued mid-turn message becomes a draft you can work on: edit in place (with the slash palette, agent commands only, because client commands act on the app *now*), reorder with ↑↓, cancel. Renders as ghost bubbles either **above the composer** or **in the transcript**, picked in Settings; inline rides SPEC-21's existing trailer row, so nothing synthetic enters `foldEvents` | SPEC-35, SPEC-21, SPEC-34 |
| [SPEC-35](./2026-08-02-SPEC-35-mid-turn-steering-and-queue.md) ([plan](./2026-08-02-SPEC-35-PLAN.md)) | **Mid-turn messages: steer vs queue** — a message typed while the agent is working is steered into the running turn where the agent has a primitive for it (codex `turn/steer`) and queued-until-idle everywhere else (ACP has none, in v1 or the v2 draft). Adds `session.queue` + `queue.cancel` and cancellable pending chips above the composer. Grounded in a live spike: a mid-turn codex `turn/start` is coerced into a steer and returns a **phantom** turn id (fixed by `87b4941`), while pi-acp queues internally and leaks the notice as agent prose | SPEC-27, SPEC-29, SPEC-33 |

# SPEC-28 — Execution plan

**Spec:** [2026-07-24-SPEC-28-desktop-workspace-tabs.md](2026-07-24-SPEC-28-desktop-workspace-tabs.md)
**Plan date:** 2026-07-24 · requires SPEC-27 merged (New-session dialog exists;
...
- `WorkspaceState {root, activeSplitId}` — **one** workspace, no per-worktree
  map (SPEC-20 revert). Ops per the spec's controller table: `divideActive`,
...
- `WorkspaceView` renders the tree; `Split` view = tab bar (active highlight,
  close/reorder, `+` → SPEC-27 dialog) above the active Tab's

# SPEC-starter-pane-parity — The starter pane keeps its work: draft text, slash commands, attachments

**Status:** Draft · **Priority:** P2 · **Branch:** `feat/new-session-remembers-input-text`
**Depends on:** SPEC-new-session-config-at-spawn (in-pane starter, cached capability catalog, apply-at-launch picks),
SPEC-desktop-workspace-tabs/SPEC-tab-groups (tabs and groups — the tab switch that loses the draft), SPEC-user-attachments (user
attachments: `POST /media`, `composerAttachmentsProvider`, `send.message attachments[]`),
SPEC-model-picker-menu-per-model-config (per-agent recents in `shared_preferences` — the persistence pattern reused here).
...
| **D10** | **A worktree that leaves the repo snapshot takes its starter draft, picks and staged images with it** ([pruneStarterDrafts], called from `desktopSessionPruneProvider` right after `closeGroupsForDeletedWorktrees`). A **closed tab does not**. The key therefore carries the project id, and the guards are **per repo**: a project missing from the snapshot is unknown, and a repo reporting no worktrees is mid-refresh *or not a git repo at all*. | These three key spaces are non-`autoDispose` by design (that is what survives the tab switch) and nothing ever dropped them: a staged screenshot for a wrapped-up worktree held megabytes for the life of the process. A closed tab is the wrong trigger — D1 promises the draft is still there when it is reopened. The project id is in the key because a worktree path cannot say which repo owns it (a linked worktree lives nowhere near its repo), and the two components are **JSON-encoded** rather than delimiter-separated. Two reviews killed two delimiters: `:` is legal in a POSIX path *and* in an id (`project-store.ts` persists any string), so `a:b`'s key read back as project `a` with a path `a` does not list — and the prune deleted a **live draft**; `\u0000` then fixed only the path half, leaving the same collision for an id containing NUL. Since a lossy key here costs the user typed text and staged images, the codec is total for every input (empty components included) and `starter_prune_test.dart` proves it by round-trip instead of arguing about which byte is safe. Keys are in-memory only, so their length does not matter. **Review caught the first version guarding globally** — bail out of the whole pass if *any* repo looked unpopulated — which `repo_service.ts` turns into "one non-git project silently disables pruning entirely", since a non-git project always reports `worktrees: []`. `closeGroupsForDeletedWorktrees` guards per repo for exactly this reason. Must run in `afterPass`: writing to other providers during a build is a Riverpod assertion, which the first version of the test caught. |
...
- **P1 (this change)** — **D1–D10**, all of it:
  - D1–D2, the draft text and the harness/model picks survive a tab switch;
  - D3–D5, the palette is populated from the per-project cache, staleness accepted;
  - D6–D7, attachments ride the spawn, gated only on a reachable server;
  - **D8**, the palette offers agent commands only — client commands are filtered out because this
    pane's send path cannot run them;
  - **D9**, a change of active server clears the cache (but boot restoring the persisted server does
    not, or the blob would be wiped on every launch);
  - **D10**, a worktree leaving the repo snapshot prunes its starter draft, picks and staged images.

  D8–D10 were added during review, after the phase plan was first written; they are P1, not deferred.
...
- Inline ACP image blocks — still SPEC-user-attachments's deferred phase.

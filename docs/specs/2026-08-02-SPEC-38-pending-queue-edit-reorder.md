...
SPEC-35 ships a pending message as a read-only chip you can only cancel. Make it a real
...
3. **Choose where they live**: `pinned` above the composer, or `inline` at the end of the
   transcript. Both ship; a preference picks (SPEC-34 precedent).
...
2. **Two placements, one widget.** `PendingQueue` is placement-agnostic and mounted twice:
   ...
   up.
3. **Inline rides the existing trailer, not synthetic items.** SPEC-21's trailer already
   occupies index 0 of the reversed list and SPEC-34's key→index map already shifts for it
...
8. **The queue stays out of the event log** (SPEC-35 decision 6). Editing a pending message
   therefore leaves no trace either — the transcript records what was *delivered*.

## Wire protocol

```ts
// New CmdKinds — both operate on the in-memory FIFO from SPEC-35.
...
- **Placement** comes from `pendingQueuePlacementProvider` (shared `store/prefs`, read by
  both surfaces — unlike SPEC-34's desktop-only navigator, a phone is exactly where a
  queue matters).
- **`pinned`** replaces SPEC-35's chip strip in the composer column.

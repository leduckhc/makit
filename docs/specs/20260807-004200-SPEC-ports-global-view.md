# SPEC-ports-global-view — Ports P2: the global view, orphans, docker, the menubar

**Status:** Draft · **Priority:** P2 · **Branch:** `feat/open-ports-fixup`
**Depends on:** SPEC-open-ports (ports P1 — scan, attribution, health, `ports.snapshot`, the row glyph,
the desktop popover, the two mobile sheets), SPEC-repo-centric-home (repo-centric home), SPEC-decomposition-and-dedup
(`ws/commands/*` split), SPEC-session-lifecycle-resume-list-delete (`archived_screen.dart` — the new-destination precedent),
SPEC-performance-metrics-dashboard (host-wide broadcast + tree walk)
...
SPEC-open-ports P1 answered *"is this branch serving anything, and is it answering?"* from the
...
feature (SPEC-open-ports's own "Why this belongs in makit"), and it is *undetectable without history*
...
| A watch-gated host broadcast (both carve-out halves) | SPEC-open-ports `ports.snapshot`, SPEC-performance-metrics-dashboard `metrics.sample` | P2b's new optional DTO fields ride the **existing** `ports.snapshot` event — no new event kind, so no new `SessionEventKind` `Exclude<>` / `HOST_ONLY_KINDS` entry is needed |
...
SPEC-open-ports owns **D1–D7**; they remain locked and P2 must not contradict them. P2's decisions
...
| D15 | **The menubar renders the app's last cached snapshot; it never arms the scanner.** The live count goes in the **tooltip** and a `Ports (n)` submenu (owned + system, plus orphans once P2b ships), with an "Open Ports…" item. No kill (P3). | An always-visible menubar count would need an always-held watch → perpetual `lsof`, breaking SPEC-open-ports's watch-gated cost discipline. And `tray_manager` exposes *icon (template PNG) + tooltip + menu* — not a rich menubar badge — so the mockup's inline "🔌 11 · 3" is **not buildable as drawn**; the count lives in the submenu label exactly like `Devices (n)`/`Sessions (n)` already do. |
...
## Phasing (this spec is SPEC-open-ports's P2, split into landable sub-phases)
...
(SPEC-open-ports's own phasing still holds: **P3** = `ports.kill` + kill-all-orphans + suggested free
...
- **No auth probing** (`no auth` pill in §6) and **no cpu/rss** (§9 legend third line) —
  killed by SPEC-open-ports D4/D5. Where the mockup shows them, they are not built.
...
- **No per-worktree filter as a first-class chip.** *This repo* is the finest built-in grain
  ...
  for.
- **No UDP, no established connections** (SPEC-open-ports's rule stands).

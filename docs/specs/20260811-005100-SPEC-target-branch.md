# SPEC-target-branch — Target branch: where a worktree's work lands

**Status:** Shipped (server + app) · **Priority:** P1 · **Branch:** `feat/base-branch`
**Number:** drafted under the retired numeric scheme. 48 and 49 were taken while this branch was in
flight (status/activity, notice layer), and 50 was claimed by profiles, hence 51 — which *preview
groups* then took as well. A spec id is now a timestamp plus a slug, so nothing has to be claimed.
See [the naming convention](./README.md#spec-naming).
**Depends on:** SPEC-pr-actions-next-step-bar (the PR next-step bar — `PrStatus`, `PrSignal`, the composer strip and the
detail sheet this reuses), SPEC-github-gateway-and-budget (the GitHub gateway and the `lastKnown` retain-on-throttle path
that makes PR-first resolution possible in the git-only phase), SPEC-repo-centric-home/SPEC-decomposition-and-dedup (the worktree

# SPEC-message-navigator — Message navigator (find your own messages in a long transcript)

**Status:** Shipped, narrowed · **Priority:** P2 · **Branch:** `feat/user-message-scroller`
**Depends on:** SPEC-chat-scroll-anchoring (reversed anchored transcript), SPEC-inline-expandable-tool-rows (fold state in `expandedTranscriptRowsProvider`)
...
2. Doing so **never** disturbs the transcript's anchoring (SPEC-chat-scroll-anchoring) or fold state (SPEC-inline-expandable-tool-rows).
...
SPEC-chat-scroll-anchoring). Two consequences drive the whole design:
...
- **Offsets of un-built rows do not exist.** Anything that draws a marker at a
  *proportional scroll position* would need the extent of rows that have never been laid
  ...
  the "measure everything" lurch SPEC-chat-scroll-anchoring removed. **Markers must therefore be placed by
...
desktop cannot drift — the SPEC-chat-scroll-anchoring/SPEC-inline-expandable-tool-rows parity rule.
...
which SPEC-model-picker-menu-per-model-config used to make the model picker's Recent list work on mobile:
...
- No new scroll physics, no replacement of `_AnchoredSliverList`, no
  `scrollable_positioned_list` dependency (it cannot preserve SPEC-chat-scroll-anchoring anchoring).
...
| 4 | Markers placed by **item index**, never scroll offset | Un-built rows have no offset (SPEC-chat-scroll-anchoring) |
...
**P3 — Outline.** Global fold reusing SPEC-inline-expandable-tool-rows's expansion machinery; hidden-row counts;
...
- **Widget:** one suite per style, driven by the preference so the tests double as the
  settings integration test. Reuse the SPEC-chat-scroll-anchoring harness.

# SPEC-desktop-chat-app — makit desktop chat app

**Status:** Phase 1 implemented · **Depends on:** SPEC-daemon-control-plane (control plane), SPEC-desktop-control-app (desktop control app), the mobile chat stack (`app/lib/{transport,store,ui}`) · **Blocks:** —

## Goal

Turn the macOS app from a **server operator's control panel** (SPEC-desktop-control-app) into a
...
future version" note in SPEC-desktop-control-app.

## Why

SPEC-desktop-control-app made the macOS build a control app because reusing the mobile app booted
...
We keep the SPEC-desktop-control-app investment (control socket client, daemon lifecycle, tray,
...
2. **Chat is the main window.** The app launches into a two-pane chat surface.
   The SPEC-desktop-control-app control screens (Devices / QR / Status / Sessions) move behind a
...
- **Settings & Server**: the SPEC-desktop-control-app control screens (Devices / QR / Status /
...
- Windows/Linux desktop — macOS only (matches SPEC-desktop-control-app).
- Any new **server** auth mode — Decision 4 reuses existing pairing.
- Multi-user / team mode.

## Design notes / contracts consumed
- **Control plane** (SPEC-daemon-control-plane) over `~/.makit/control.sock`: verbs `status`

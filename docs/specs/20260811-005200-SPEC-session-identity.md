# SPEC-session-identity — Session identity: copy the id, and its transcript
...
> **Renumbered twice, then renamed.** This spec was drafted as number 51, under the retired
> numeric scheme. Two other specs took the numbers around it on `main` while it was in flight:
> *Preview groups* took 51 (#163, referenced from `docs/UX.md` and fourteen shipped source files)
> and *Profiles* took 50 (#162). Both shipped first and neither could move, so this one took 52.
> The branch name (`feat/get-session-id`) is deliberately left alone.
> Renaming a pushed branch would orphan its PR for no gain.
>
> Renumbering never fixed the hazard, and it could not. Nothing in the repo allocated a number,
> so two branches drafted in the same week collided silently. `main` carried the proof: one
> number named *both* SPEC-status-and-activity and SPEC-per-repo-settings.
> A spec id is now a creation timestamp plus a slug, so no two worktrees can collide.
> See [the naming convention](./README.md#spec-naming).

**Depends on:** SPEC-session-lifecycle-resume-list-delete (`agentSessionId` / `resumeSessionPath` persistence, closed-session
resume), SPEC-context-usage (`ContextUsageDetails` — the panel this appends to, and the ring's absence
rule this must not weaken), SPEC-session-timings D12 (the precedent for adding one optional field to
`SessionDTO` rather than a new command), SPEC-mid-turn-steering-and-queue (the mid-turn queue — the thing that makes
...
- **`Session.agentSessionId`** (`server/src/session.ts:163`) — the native ACP `sessionId` (set
  ...
  resume handle by SPEC-session-lifecycle-resume-list-delete.
...
| **D9** | A row whose value is unmeasured is **omitted**, never rendered blank or as a placeholder. `Copy all` then copies fewer lines. | Same rule SPEC-context-usage settled on for the ring. A fabricated path is worse than no path: it will be pasted into a prompt and the next agent will report it missing. |
...
| **D20** | Labels are hard-coded English, singular-only (`pi session`, `Transcript`, `Resume with`, `Copy all`). | Mirrors SPEC-session-timings D20 verbatim. Called out so review does not re-open it. |
...
| `agentSessionId` persistence + rehydration | SPEC-session-lifecycle-resume-list-delete, `storage/sqlite_event_store.ts:142` | a new table or a live-only field |
| optional-field-on-`SessionDTO` pattern | SPEC-session-timings D12 (`createdAt`) | a new `cmd` kind |
...
| No accessibility decision at all, where SPEC-session-timings locked one. | **Accepted — D18**, with a widget test. |
...
| i18n divergence from SPEC-session-timings D20. | **Accepted — D20.** |
...
2. `cd app && flutter analyze --fatal-infos --no-pub` → "No issues found!";
   ...
   regression. The 71 SPEC-session-identity tests are green run as a set. ✅

# SPEC-pr-actions-next-step-bar — PR actions: the next-step bar

**Status:** Implemented; every gap in §11 closed, with 3 accepted limitations in §11a · **Priority:** P2 · **Branch:** `feat-pr-actions`
**Depends on:** SPEC-pr-status-and-actions (composer PR bar + canned prompts — this replaces its UI),
SPEC-github-gateway-and-budget (GitHub gateway: `PrLookup`, quota ladder, cache), SPEC-repo-centric-home (repo-centric mobile home),
SPEC-decomposition-and-dedup (worktree lifecycle)

**Review record:** [`20260806-003800-SPEC-pr-actions-next-step-bar-REVIEW.md`](./20260806-003800-SPEC-pr-actions-next-step-bar-REVIEW.md) — five passes,
...
## 2 · The problem with SPEC-pr-status-and-actions's bar
...
The CTA is the first matching row, top down, and preserves SPEC-pr-status-and-actions's relative order for the rows it
...
Rows inherited unchanged from SPEC-pr-status-and-actions's `_situationFor()` are **1, 2, 3, 5 and 7** (uncommitted,
...
# 2. THEN reconcile sessions bound to it (SPEC-session-lifecycle-resume-list-delete): live → archived (transcript and
...
- **Stale** (SPEC-github-gateway-and-budget §7.5) dims only the *derived* half and appends `· last known` with the existing
  tooltip. The PR number never goes stale; SPEC-pr-status-and-actions's pill dimmed wholesale, hiding the one reliable fact.
...
- **A verdict with no checks says why.** `checkRollup != 'none'` with an empty `checks` list is a shed
  lookup (SPEC-github-gateway-and-budget drops the second call under quota), so the detail prints `kShedChecksNote` instead of
...
A fresh server-side lookup before acting was considered and **rejected**: under SPEC-github-gateway-and-budget's quota ladder
...
2. **A stale snapshot is declared.** When `PullRequest.stale` is set (SPEC-github-gateway-and-budget shed the refresh), the
...
| D2 | The CTA **rests** when nothing is pressing | SPEC-pr-status-and-actions fell back to the last-picked action, offering Pull with nothing to pull |
...
| D18 | A **shed** rollup still reports its verdict | `pending`/`fail` come from `checkRollup`, but the *count* comes from the check list, and SPEC-github-gateway-and-budget sheds the list under quota. Deriving "in flight" from the list alone made a running build indistinguishable from an all-clear — and the all-clear branch then offered `Squash & merge` on a build that had not finished. A countless `CI still running` and `PrDot.pending` now cover it; `checkProgress` stays null, so the dot is a solid disc rather than an arc claiming a count it does not have |
...
| D22c | The `refs/heads` watch **re-arms from its own error handler** | `fs.watch` reports a runtime failure (an inotify limit, the directory going away) asynchronously, and the guard closes the watcher when it does. With the key left in place nothing re-attached it — and `sync()` will not re-arm a repo whose path has not changed — so the repo silently lost its only commit trigger for the life of the process. Both paths now go through one `armHeads`, which drops the key and re-arms, falling through to the per-directory walk where recursive watching is no longer available. Not reachable in a test on darwin: `fs.watch` there survives the watched directory being deleted and recreated without ever emitting `error` |

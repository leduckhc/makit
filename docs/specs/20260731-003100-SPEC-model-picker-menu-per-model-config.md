# SPEC-model-picker-menu-per-model-config — Model picker menu + per-model config flyout
...
- `app/lib/ui/composer/composer_selectors.dart` — `ConfigOptionPickRow`,
  `ConfigOptionPill` (SPEC-new-session-config-at-spawn), `ComposerConfigOptions`, `ThinkingSignal`, and
...
5. **New-session dialog parity.** `worktree_starter.dart` uses the same menu/flyout
   ...
   re-emit barrier) — unchanged from SPEC-new-session-config-at-spawn.
...
- **Re-emit is a full replacement.** Every `configOption` set re-emits the
  ...
  wholly from it (never merge) — already the SPEC-acp-config-options-unified-composer/27 contract. An option that

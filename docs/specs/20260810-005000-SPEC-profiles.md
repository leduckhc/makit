# SPEC-profiles — Profiles: one server per purpose, and a Server panel you can read
...
**Depends on:** SPEC-status-and-activity (`app/lib/status/` — used as the failure sink), SPEC-desktop-settings-rework (settings section layout)

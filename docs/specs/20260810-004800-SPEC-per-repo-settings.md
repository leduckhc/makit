# SPEC-per-repo-settings — Per-repo settings: one Settings section per repository

**Status:** P2 Implemented (rev 3.2) · **Priority:** P2 · **Branch:** `feat/forgejo-git-provider`
**Depends on:** SPEC-repo-centric-home (repo-centric home — `RepoDTO`, `repos.snapshot`, the repo card and its
`dotsThree` menu), SPEC-decomposition-and-dedup (`SettingsResetButton` as the one shared "reset to default" widget, and
...
gates a privileged input — the app's reported pid in `hello` (SPEC-performance-metrics-dashboard decision 6): *"a non-loopback
...
| **D16** | **Only a loopback client may write per-repo settings.** A non-loopback client receives an explicit refusal, not a silent no-op. Reads are unrestricted. | `worktreeRoot` is a path the daemon creates directories under and, via prune, removes. A remote device that can set it directs host filesystem operations. Precedent: SPEC-performance-metrics-dashboard D6 / `WsClient.isLocal`. |
...
(SPEC-github-gateway-and-budget §6.5) — the exact chatter `None` exists to stop. Pinned by a test

# SPEC-composer-footer-space — Composer footer: space by need

**Status:** Implemented · **Priority:** P2 · **Branch:** `fix/token-usage` (continues)
**Plan:** [`20260806-004000-SPEC-composer-footer-space-PLAN.md`](./20260806-004000-SPEC-composer-footer-space-PLAN.md)
**Mockup:** [`mockups/composer-footer-space.html`](../../mockups/composer-footer-space.html)
**Depends on:** SPEC-acp-config-options-unified-composer (unified config options), SPEC-model-picker-menu-per-model-config (model pill + picker), SPEC-context-usage *(context usage — the ring that shares this row)*
...
*docs:* this spec + plan, the SPEC-context-usage crowding correction
(`20260805-003700-SPEC-context-usage.md`), `docs/specs/README.md`.
...
## The premise SPEC-context-usage got wrong

SPEC-context-usage recorded the footer as crowded: *"the footer's pill area is 267 pt, and a 3-config-pill
...
four-option shape still overflowed at 320 pt. The chips summarise model-scoped options (SPEC-model-picker-menu-per-model-config)
...
off-screen; the ring's whole purpose is being glanceable without a tap (SPEC-context-usage §Why a ring).
...
- Redesigning the picker sheets or `ModelPickerMenu` (SPEC-model-picker-menu-per-model-config owns those).
...
- Correcting SPEC-context-usage's `32 pt`/`36 pt` discrepancy beyond this spec's own text: the control is
  36 pt in code, SPEC-context-usage describes it as 32 pt in prose. Recorded here; the code is the truth and
  is not changed.

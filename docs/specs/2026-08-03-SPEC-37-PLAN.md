# SPEC-37 — Implementation plan

Spec: [`2026-08-03-SPEC-37-performance-metrics-dashboard.md`](./2026-08-03-SPEC-37-performance-metrics-dashboard.md)
...
export function sumTree(table, index, root): { rssBytes: number; cpuSeconds: number; procs: number };
```
...
```
- Keep `credited: Map<pid, seconds>` **per root**.
- Total = `Σ over pids-seen-this-tick (row.cpuSeconds)` **plus** `Σ over
  pids-that-vanished (their last credited value)`. Vanished pids stay in the map as
  a frozen contribution — that is what makes the total monotonic when ripgrep exits.
- A pid **reused** by the OS for an unrelated process would double-count. Guard by
  also storing `comm` and dropping the frozen entry if a pid reappears with a
  different `comm`. Cheap, and pid reuse over a 30-min window is real.
- Never decrease the returned total; assert this in the test.

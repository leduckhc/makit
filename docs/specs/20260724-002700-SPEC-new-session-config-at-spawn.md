# SPEC-new-session-config-at-spawn — New-session configuration at spawn (worktree · harness · config options)

**Status:** Proposed · **Priority:** P2 · **Surface:** desktop chat starter (`app/lib/desktop/chat/`), server (`server/src/`), wire protocol
**Depends on:** SPEC-acp-config-options-unified-composer (unified `configOptions` model + composer renderer this dialog reuses), SPEC-desktop-chat-app (desktop chat app). **Supersedes** SPEC-session-in-pane-spawning's spawn path (session-in-pane) — all sessions are headless (no attachable pane).
...
(SPEC-acp-config-options-unified-composer). So the `native` transport stays for codex; the descriptor
...
- A harness's options arrive **only after a session exists** — ACP returns
  `configOptions` in the `session/new` response (SPEC-acp-config-options-unified-composer). There is nothing to
...
1. **Config lives in a dedicated "New session" dialog.** A single modal
   ...
   **replaces both** today's `WorktreeStartView` (inline "start in an existing
   ...
   branch/PR"). SPEC-desktop-workspace-tabs's inline starter Tab is removed (see SPEC-desktop-workspace-tabs update).
...
3. **Config = the harness's unified `configOptions`.** Every harness exposes a
   ...
   same shape (SPEC-acp-config-options-unified-composer). There is **no** separate native model/thinking model
...
4. **Config comes from a cached catalog — no eager session (frozen).** A
   ...
   **ACP** → `session/new` in an empty temp `cwd`, options captured, session
   ...
   session, no worktree, no teardown races — and renders it via SPEC-acp-config-options-unified-composer's
...
> and pi's **mux-pane** path (`manager.spawnPiSessionInPane`, SPEC-session-in-pane-spawning). pi now
...
- **Flutter**
  ...
  - `app/lib/ui/composer/composer_selectors.dart` — the composer's config
    selectors; SPEC-acp-config-options-unified-composer unifies these into one `configOptions` renderer this
    dialog reuses.
  ...
  - `app/lib/store/models.dart` — `AgentDescriptor`, `SessionMeta`,
    `SessionConfigOption` (SPEC-acp-config-options-unified-composer).
- **Server / protocol**
  ...
  - `server/src/adapters/acp.ts` — the ACP adapter pi runs through; the ACP
    ...
    (SPEC-acp-config-options-unified-composer).
...
interface AgentDescriptor {
  ...
  // codex: app-server surface projection). SPEC-acp-config-options-unified-composer shape:
  // {id, name, category, type, currentValue, options?/groups?}. Rendered by
  // SPEC-acp-config-options-unified-composer's generic renderer; picks applied at launch. Absent/empty → default-only.
...
- **Probe (only on miss/refresh), per transport.** **ACP** (pi): launch the
  ...
  **codex `app-server`:** query the model/reasoning surface (no user thread
  retained) and **project** it into `configOptions` (SPEC-acp-config-options-unified-composer). **cwd caveat:**
...
- The **New session dialog** (evolved `new_session_dialog.dart`) is laid out
  ...
  4. **First message** composer (reusing `Composer`). The harness's config
     ...
     from the cached `configOptions` catalog via **SPEC-acp-config-options-unified-composer's generic category
  ...
  - Selecting a harness swaps the buttons to that harness's cached
    ...
    **Tab in the active split** once SPEC-desktop-workspace-tabs lands; today's selected pane before
  ...
  **Every empty pane reaches this dialog (interim, until SPEC-desktop-workspace-tabs's starter
...
push-navigation flow (no tabs/splits — SPEC-desktop-workspace-tabs is desktop/iPad-only). The sheet
...
- **Config options** — tappable rows rendering the harness's cached
  `configOptions` (model / thought_level / mode / …) via SPEC-acp-config-options-unified-composer's renderer;
...
5. **New session dialog UI (desktop/iPad).** Widget tests: the Worktree source
   ...
   via the SPEC-acp-config-options-unified-composer renderer; switching harness swaps the buttons; **send** calls
...
- **Loss of the mux pane (SPEC-session-in-pane-spawning).** pi no longer runs in an attachable
  terminal pane; all sessions are headless (pi via ACP, codex via app-server).
  SPEC-session-in-pane-spawning's pi-in-pane path and the README “attach to the pane” story are
  **retired** — update/close SPEC-session-in-pane-spawning accordingly.
...
- Changing how config switches on an already-running session (the SPEC-acp-config-options-unified-composer
  composer selectors are unchanged).
- Formally retiring SPEC-session-in-pane-spawning's mux-pane path is a consequence tracked here;
  ACP fs sandboxing is **explicitly not planned** (accepted).
- **Mobile:** the tab/split layer (SPEC-desktop-workspace-tabs) does not apply; mobile keeps

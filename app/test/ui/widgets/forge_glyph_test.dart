...
void main() {
  group('forgeKindForUrl', () {
    ...
    test('is not fooled by a lookalike host', () {
      ...
      // classify an attacker-controlled host as GitHub.
      expect(
        forgeKindForUrl('https://github.com.evil.test/a/b/pull/1'),
        ForgeKind.forgejo,
      );
      expect(
        forgeKindForUrl('https://notgithub.com/a/b/pull/1'),
        ForgeKind.forgejo,
      );
    });
    ...
    test('treats any other host as Forgejo, matching the server router', () {
      // The server sends every non-GitHub remote to its Forgejo provider, so the
      // glyph must agree with the provider that actually served the data.
      expect(

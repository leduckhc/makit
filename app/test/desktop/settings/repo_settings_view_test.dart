...
void main() {
  test(
    'a server that sends no settings yields no view, not fabricated defaults',
    () {
  ...
  test('a forge we cannot talk to maps to no glyph, not to a wrong one', () {
    for (final software in ['gitlab', 'unknown', 'bitbucket']) {
      ...
      expect(
        v.forgeHost,
        'h',
        reason: 'the host still shows, so the row can name it',
      );
    }
  });
  ...
  test('no remote and no detection are DIFFERENT states', () {
    final noRemote = repoSettingsViewFor(
      _repo({
        'worktreeRoot': _root,
        'provider': {'value': 'auto', 'source': 'default'},
        'hasRemote': false,
    ...
    final pending = repoSettingsViewFor(
      _repo({
        'worktreeRoot': _root,
        'provider': {'value': 'auto', 'source': 'default'},
        'hasRemote': true,
  ...
  test(
    'the provider choice round-trips, unknown wire values falling back to auto',
    () {
      ForgeChoice choiceFor(String wire) => repoSettingsViewFor(
        _repo({
          'worktreeRoot': _root,
